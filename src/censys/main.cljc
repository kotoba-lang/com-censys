(ns censys.main
  "Kotodama WASM entrypoint for the Censys clean-room actor (L4) — Clojure port.
  ...
  facts (`censys.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "censys")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"censys.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "censys-compat" :tier tier :entities entities} 200])
...
(defn start! [] :censys-compat/ready)
